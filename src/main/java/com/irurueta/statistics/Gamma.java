...
public class Gamma extends GaussLegendreQuadrature{
     ...
     */
    public static final double FPMIN = Double.MIN_VALUE / EPS;
    
    /**
     * Maximum value supported to estimate factorials for
     */
    public static final int MAX_FACTORIALS = 170;
    
    /**
     * Maximum number of logarithm of factorials cached
     */
    public static final int MAX_CACHED_LOG_FACTORIALS = 2000;
    ...
    public static double gammln(double xx) throws IllegalArgumentException{        
        if(xx <= 0.0) throw new IllegalArgumentException("bad arg in gammln");
	...
	if (n < MAX_CACHED_LOG_FACTORIALS) return logarithmOfFactorialsTable[n];
	return gammln(n+1.);
    }
	...
	for (;;) {
            ...
            if(ap >= maxIterations) throw new MaxIterationsExceededException();
        }
    }
	...
	for (i=1;;i++) {
            ...
            if(i >= maxIterations) throw new MaxIterationsExceededException();
	}
